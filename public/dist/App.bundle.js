...
function searchResultsHTML(timesheets) {
  return timesheets.map(function (project) {
...
function dateSearch(date) {
  if (!date) return;
  var dateInput = date.querySelector('input[name="date"]');
  var dateForms = date.querySelector("div.dateForm");
  var timeForm = date.querySelector(".timesheet");
  var appendForm = date.querySelector("#tbl");
  var postForm = date.querySelector("#post");
  ...
  dateInput.on("input", function () {
    ...
    dateForms.style.display = "block";
    _axios2.default.get("/api/timesheets?date=" + this.value).then(function (res) {
      if (res.data.length) {
        var datepic = searchResultsHTML(res.data);
      ...
      if (!res.data.length) {
        alert("No results for " + _this.value);
...
function post(e, datas) {
  ...
  var project = document.getElementById("protitle");
  var protitle = project.options[project.selectedIndex].text;

  var story = document.getElementById("storytitle");
  var storytitle = story.options[story.selectedIndex].text;

  var task = document.getElementById("tasktitle");
  var tasktitle = task.options[task.selectedIndex].text;
  var desc = document.getElementById("desc").value;
  var time = document.getElementById("time").value;

  _axios2.default.post("/timesheets", {
...
var _dateSearch = __webpack_require__(11);

var _dateSearch2 = _interopRequireDefault(_dateSearch);
...
(0, _autocomplete2.default)((0, _bling.$)('#protitle'), (0, _bling.$)('#tasktitle'), (0, _bling.$)('#storytitle'));

(0, _dateSearch2.default)((0, _bling.$)(".postData"));

var taskForms = (0, _bling.$$)(".postData");
taskForms.on("submit", _post2.default);
